...
def find_food(ingredients, excludeIngredients):
    ...
    def create_new_combination(ingredients, excludeIngredients):
        data = requests.get(
            f"https://api.spoonacular.com/recipes/complexSearch?includeIngredients={','.join([str(item) for item in ingredients])}&excludeIngredients={','.join([str(item) for item in excludeIngredients])}&fillIngredients=true&sort=min-missing-ingredients&addRecipeNutrition=true&number=5&apiKey=499e83f1ec4c45b18cea15d1d236cd1b").json()
...
if __name__ == '__main__':
    ...
    recipe = json.loads(find_food(['patato','cheese'], ['onion'])[2][0][0])
    create_page(['patato','cheese'], ['onion'], recipe)
